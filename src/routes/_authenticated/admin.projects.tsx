...
const EMPTY = {
  ...
  slug: "", title: "", description: "", brands: "", service_area: "",
  image_urls: [] as string[], completed_on: "", is_published: true,
};

function ProjectsAdmin() {
  const qc = useQueryClient();
  const { data = [] } = useQuery({ queryKey: ["admin-projects"], queryFn: () => adminListProjects() });
  ...
  function edit(p: any) {
    setForm({
      ...
      brands: (p.brands || []).join(", "), service_area: p.service_area || "",
      image_urls: p.image_urls || [], completed_on: p.completed_on || "", is_published: p.is_published,
    });
    setOpen(true);
  }
  ...
  async function remove(id: string) {
    ...
    try { await adminDeleteProject({ data: { id } }); qc.invalidateQueries({ queryKey: ["admin-projects"] }); }
    ...
    <div>
      <header className="mb-8 flex items-center justify-between">
        <div>
          ...
          <p className="mt-2 text-sm text-muted-foreground">Portfolio entries shown on the public site.</p>
        </div>
        <Button onClick={() => { setForm(EMPTY); setOpen(true); }}><Plus className="mr-2 h-4 w-4" /> New project</Button>
      </header>
          ...
          <div key={p.id} className="flex items-center justify-between rounded-lg border border-border bg-card p-4">
            <div>
              <div className="flex items-center gap-2">
                ...
                {!p.is_published && <span className="rounded bg-muted px-1.5 py-0.5 text-[10px]">draft</span>}
              </div>
              <div className="mt-1 text-xs text-muted-foreground">/{p.slug} · {p.service_area ?? "—"} · {p.brands.join(", ")}</div>
            </div>
        ...
        <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
          ...
          <div className="grid gap-4">
            <div className="grid gap-4 md:grid-cols-2">
              ...
              <div><Label>Completed on</Label><Input type="date" value={form.completed_on} onChange={(e) => setForm({ ...form, completed_on: e.target.value })} /></div>
            </div>
              ...
              <div className="mt-2 flex flex-wrap gap-2">
                {form.image_urls.map((u, i) => (
                  <div key={u} className="relative">
                    ...
                    <button type="button" onClick={() => setForm({ ...form, image_urls: form.image_urls.filter((_, j) => j !== i) })} className="absolute -right-2 -top-2 grid h-5 w-5 place-items-center rounded-full bg-destructive text-destructive-foreground">
                      <X className="h-3 w-3" />
                    </button>
                  </div>
                ))}
