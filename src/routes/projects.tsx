  ...
  head: () => ({
    meta: [
      { title: "Recent Repair Projects | Sub-Zero, Viking & Wolf | NY & NJ" },
      {
        name: "description",
        content:
          "A sample of premium appliance repair projects completed for customers across Staten Island, Brooklyn, Long Island and New Jersey by our repair specialists.",
      },
      { property: "og:title", content: "Recent Repair Projects" },
      { property: "og:description", content: "Sub-Zero, Viking, Wolf and Thermador projects we've completed." },
...
function ProjectsPage() {
  ...
  const brands = useMemo(() => Array.from(new Set(projects.flatMap((p) => p.brands))).sort(), [projects]);
    ...
    <div>
      <section className="border-b border-border">
        <div className="mx-auto max-w-7xl px-4 py-16 md:px-8 md:py-20">
          <h1 className="text-4xl font-semibold tracking-tight md:text-5xl">Recent <span className="text-accent">projects</span></h1>
          <p className="mt-4 max-w-2xl text-muted-foreground">
            A snapshot of repairs we've completed. Every job starts with an accurate diagnostic and a transparent estimate.
          </p>
          <a
      ...
      <section className="mx-auto max-w-7xl px-4 py-10 md:px-8">
        <div className="flex flex-wrap gap-2">
          <Button size="sm" variant={brand === null ? "default" : "outline"} onClick={() => setBrand(null)}>All brands</Button>
          {brands.map((b) => (
        ...
        <div className="mt-10 grid gap-6 md:grid-cols-2 lg:grid-cols-3">
          {filtered.map((p) => (
            <article key={p.id} id={p.slug} className="overflow-hidden rounded-lg border border-border bg-card">
              <ImagePlaceholder
                aspect="video"
                label={p.title}
                src={p.image_urls?.[0]}
              ...
              <div className="p-5">
                <div className="flex flex-wrap gap-1">
                  ...
                  {p.service_area ? <Badge variant="outline" className="font-normal">{p.service_area}</Badge> : null}
                </div>
