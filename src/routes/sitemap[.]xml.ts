...
import { DB_STUBBED, mockServices, mockProjects } from "@/lib/db-stub";
    ...
    handlers: {
      GET: async () => {
        ...
        if (DB_STUBBED) {
          ...
          const [{ data: svc }, { data: proj }] = await Promise.all([
        ...
        const urls = [
          ...
          ...services.map((r) => ({ loc: `/services/${r.slug}`, priority: "0.7", changefreq: "monthly" as const })),
          ...projects.map((r) => ({ loc: `/projects#${r.slug}`, priority: "0.5", changefreq: "monthly" as const })),
          ...BLOG_POSTS.map((p) => ({ loc: `/post/${p.slug}`, priority: "0.6", changefreq: "monthly" as const })),
        ];
